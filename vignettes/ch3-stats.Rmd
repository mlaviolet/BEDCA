...
The result is assigned to an object `lm_null`, which contains all information about the fitted model. The information can be extracted in various ways; the `mosaic::msummary()` operator produces a brief report. Note that the argument is the model object.

```{r}
mosaic::msummary(lm_null)
```

The formula specifies the structure of the model. The specific numbers for the model computed from the data are called *coefficients*. The null model has only one coefficient: the mean response, which in this case is 1.495. The coefficient is labelled `(Intercept)` for reasons you'll see in Section 4.1.
The line `Residual standard error: 1.39 on 5 degrees of freedom`, for this model, means that the standard deviation is 1.39 and that the divisor for finding the mean square is 5; that is, 6 - 1. That's all we need for now.
