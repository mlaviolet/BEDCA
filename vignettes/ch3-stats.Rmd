...
where `y` is the variable to be plotted and `z` is a panel variable; that is, separate plots are produced for each level of `z`. The tilde `~` is part of the syntax for a *modeling formula*, which will be explained shortly.
...
This code produces a dot diagram similar to Figure 3.2, page 68. The `nint` argument specifies the number of intervals to use.
...
For a quick look at the frequency distribution, use the `summary()` operator on the `grp` variable.

```{r}
summary(gears_laid$grp)
```

Now compute the frequencies, relative frequencies, and cumulative relative frequencies from Table 3.2 on page 70:
...
Figure 3.6 (p. 72) is a histogram of penetration depths for the 200-grain bullets. The `gf_histogram()` function does the work of dividing the measurements into intervals. Usually the function makes good choices but we can refine the graph by specifying the intervals with the `breaks` argument. We can also specify the number of intervals with the `bins` argument and the interval length with `binwidth`.

```{r}
ggformula::gf_histogram(~ G200, data = table3_01,
                        breaks = seq(58, 72, 2),
                        xlab = "Penetration depth (mm)")
```

We can also do comparative histograms with the reshaped data. By default the scales of both graphs are the same.
...
-   The whiskers extend from the ends of the box to the minimum and maximum, unless any values are more than 1.5 box lengths from the nearest end of the box. In that case, the whiskers extend only to the most extreme value within 1.5 box lengths and the values outside that range are plotted separately as outliers.
...
Note: The default behavior of `gf_qq()` places the data on the vertical axis and the normal quantiles on the horizontal, which is the opposite of the text. The `coord_flip()` function will replicate the format in the text. Note that the labels are flipped along with the points.
...
R uses *formulas* to define models. The general format for a formula is

$$\texttt{y} \sim \texttt{x}$$

where `y` is the response variable and `x` is one or more predictors. We'll start with the simplest possible model: one with no predictors. This model is called the *null model*. Developing procedures for the null model will help us to understand more complex models.
...
Another key observation is that some values are greater than the mean while others are less. The differences between each value and that predicted by the model are called the *residuals*. In other words, we can partition each data value as
...
For the null model the residual mean square has a special name: the *variance*. So in this case, our predicted waste is 1.49 percent and the variance is 1.94 ... what? The units of variance are the squared units of the data, which is not intuitive. To get back to the original units, take the square root.
...
As noted in the text on page 98, at least 75% of a data set always falls within two standard deviations of the mean; at least 89% always falls within three standard deviations. The smaller the standard deviation, the more tightly the data are packed about their mean.
...
Other summary statistics are computed similarly.

Now we'll learn the specialized tools for fitting models. We'll begin with the null model and later apply these methods to more complex models.

The operator for fitting models is `lm()` for "linear model." The syntax is

$$\texttt{lm(y}\sim \texttt{x, data = mydata, ...)}$$

The first argument is the model formula: `y` is a response variable and `x` is one or more predictor variables. `mydata` is the data set containing`y` and `x`, and the triple dot `...` represents additional arguments as needed.
...
The result is assigned to an object `lm_null`, which contains all information about the fitted model. We can extract the information in various ways. Printing the model object gives bare-bones information.

```{r}
lm_null 
```

The `Call` section repeats the model we just fitted. The `Coefficients` section gives the numbers computed according to the least squares principle. The null model has only one coefficient: the mean response, shown here as 1.5. The coefficient is labelled `(Intercept)` for reasons you'll see in Section 4.1.

The `mosaic::msummary()` operator gives more information. Note that the argument is the model object.

```{r}
mosaic::msummary(lm_null)
```

The sole coefficient is shown here as 1.495. The line `Residual standard error: 1.39 on 5 degrees of freedom` means that the standard deviation of the data values is 1.39 and the divisor for finding the mean square is 5; that is, $6-1$. That's all we need for now.
...
Now we'll examine the fitted values and residuals, using the `augment()` operator from the `broom` package. This will create containing fitted values, residuals and other information about the model data.

```{r}
broom::augment(lm_null)
```

We only need the first three columns: the first contains the data values, the `.fitted` column contains the model fit, and `.resid` contains the residuals. In the null model, the values for `.fitted` are all the same; specifically the mean response. In Chapter 4, when we fit more interesting models, we'll see how to use this model data.
...
Example 9 (p. 101) discusses a student experiment studying the strength of glued wood joints with three woods and three glues. Figure 3.22 (p. 102) shows an *interaction plot*, which examines the pattern of differences between woods for each type of glue. The plot is a line graph of responses against the levels of one factor (*x factor*), grouped by levels of a second factor (*trace factor*).

The following code replicates Figure 3.22 (p. 102). The variable to be plotted is `mean_strength`, `glue` is the x factor, and `wood` is the trace factor. First we change the character variables to factors.

```{r}
wood_joint <- table3_15 |> 
  # make wood and glue "factors" 
...
ggformula::gf_point(mean_strength ~ glue, data = wood_joint,
                    # glue is x factor
                    group = ~ wood, # wood is trace factor
                    shape = ~ wood) |> # different symbol for each wood
  ggformula::gf_line(linetype = ~ wood, # different line type for each wood
                     show.legend = TRUE,
                     xlab = "Glue type",
                     ylab = "Strength (lb)") +
  # tweak aspect ratio and angle glue type labels
...
Bar charts are often used for categorical data. Figure 3.23 (p. 108) presents defects in cable connectors by well-defined categories. The operator `gf_col()` is used when the data consist of counts; while `gf_bar()`will do the counting when the data consist of individual items.

```{r}
connectors <- table3_16 |> 
  mutate(p = connectors / 3000)

ggformula::gf_col(p ~ category, data = connectors,
                  fill = "steelblue", # bar color
                  xlab = "Connector category",
                  ylab = "Fraction of connectors")
```

Figure 3.24 (p. 108) is a bar chart of manufactured tools with various problems. The graph shows that leaks are the most frequently occurring problems.

```{r}
tools <- table3_17 |> 
  mutate(p = n_tools / 100)

ggformula::gf_col(p ~ problem, data = tools,
                  fill = "steelblue",
                  xlab = "Problem rate") + 
  # this is a tweak to place the tick labels at an angle
  #   so that they're easier to read
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```

Figure 3.25 (p. 110) presents a segmented bar chart, which is used to compare categories. Here, the percentages requiring both scrap and rework are presented in a single bar for each job using the `fill` argument of `gf_col()`.

```{r}
ggformula::gf_col(pct ~ job, data = table3_18,
                  fill = ~ type, # different colors for each type
                  xlab = "Job number",
                  ylab = "Percent of production")
```

Figure 3.26 (p. 111) presents a line chart of defects per day in trucks produced at an assembly plant.

```{r}
ggformula::gf_point(defects_truck ~ day, data = table3_19,
                    xlab = "Day",
                    ylab = "Defects/truck") |>
  gf_line() +
  theme(aspect.ratio = 0.35)
...
Figure 3.27 (p. 112) is an interaction plot from a study of quality of pellets produced by a pelletizing machine.

```{r}
pellets <- table3_20 |> 
  mutate(shot_size = factor(shot_size, levels = c("small", "large")),
         p = n_conform / 100)

ggformula::gf_point(p ~ shot_size, data = pellets,
                    group = ~ mixture,
                    shape = ~ mixture) |>
  ggformula::gf_line(linetype = ~ mixture, 
                     show.legend = TRUE,
                     xlab = "Shot size",
                     ylab = "Fraction of pellets nonconforming") 
```

### Summary

A single quantitative variable can be summarized graphically by

-   Dot plots, which show the individual values

-   Histograms, which group the data into bins. Histograms show the shape, center, and spread of a distribution

-   Run charts, which show values in time order of collection

Scatterplots are used to show relationships between two quantitative variables

Boxplots are used to compare distributions of quantitative variables at the different levels of a categorical variable.

### Appendix

We've seen how our model works in computer terms. In mathematical terms, the null model is represented as

$$y \approx \mu$$

read as "$y$ is approximately equal to $\mu$, where $\mu$ (Greek "m") is a hypothetical constant. Hypothetical quantities are typically written as Greek letters.

A sample of quantitative or continuous data consists of values $y_1, y_2, \ldots , y_n$. The *sample mean* is

$$\text{Mean: } \bar{y} = \frac{1}{n} (y_1 + y_2+ \ldots + y_n) = \frac{1}{n} \sum_{i=1}^{n}y_i$$

The variance and standard deviation are

$$\text{Variance: }s^2=\frac{1}{n-1} \sum_{i=1}^{n}(y_i-\bar{y})^2$$

$$\text{Standard deviation: } s = \sqrt{s^2}$$

The value of $\mu$ estimated from the data is the sample mean $\bar{x}$. When we fit the null model to a single variable, the model coefficient is the mean and the residual standard error is the standard deviation.
