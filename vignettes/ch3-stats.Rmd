...
The `augment()` operator from the `broom` package creates a date set containing the fitted values and residuals. 

```{r}
broom::augment(lm_null)
```

We only need the first three columns: the first contains the data values, the `.fitted` column contains the model fit, and `.resid` contains the residuals. In Chapter 4, when we fit more interesting models, we'll see how to use this model data.
