...
For the null model the residual mean square has a special name: the *variance*. So in this case, our predicted waste is 1.495 percent and the variance is 1.945 ... what? The units of variance are the squared units of the data, which is not intuitive. To get back to the original units, take the square root.
...
Other summary statistics are computed similarly. The mathematical formulas for the sample mean and standard deviation are:
...
The result is assigned to an object `lm_null`, which contains all information about the fitted model. The information can be extracted in various ways. Printing the model object gives bare-bones information.
...
read as "$y$ is approximately equal to $\mu$, where $\mu$ (Greek "m") is some constant. In the R language, the model is fitted with the `lm()` function.

$$\texttt{lm(y}\sim \texttt{1)}$$

The model fitted from the data is

$$\texttt{pct_waste = 1.495}$$

Now we'll examine the fitted values and residuals, using the `augment()` operator from the `broom` package. This will create a data set containing fitted values, residuals and other information about the model data.
