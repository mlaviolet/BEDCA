...
We can also do comparative histograms with the reshaped data. To put both graphs on the same scale, specify `breaks` to cover the range of depths for both tipes.
...
The result is assigned to an object `lm_null`, which contains all information about the fitted model. The `summary()` operator produces a report on the fitted model. Note that the argument is the model object.
...
The following code replicates Figure 3.22 (p. 102). In the `gf_line()` operator, `mean_strength` is the variable to plotted, `glue` is the x-factor, and `wood` is the trace factor. First we change the character variables to factors.
...
ggformula::gf_point(mean_strength ~ glue, data = wood_joint,
                    group = ~ wood,
                    shape = ~ wood) |>
  ggformula::gf_line(linetype = ~ wood, 
                     show.legend = TRUE,
                     xlab = "Glue type",
                     ylab = "Strength (lb)") +
...
Figure 3.24 (p. 108) is a bar chart of manufactured tools with various problems. Leaks are the most frequently occurring problems.
