---
title: "Chapter 4, Part 2: Fitted Effects for Factorial Data"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Chapter 4, Part 2: Fitted Effects for Factorial Data}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.align="center"
  )
```

```{r setup, message=FALSE}
library(BEDCA)
library(mosaic)
library(tidyverse)
library(emmeans)
library(broom)
theme_set(theme_bw())
options(digits = 5)
```

## 4.3: Fitted Effects for Factorial Data

### 4.3.1: Fitted Effects for 2-Factor Studies

Introduce effects with example in Vardeman-Jobe SMQA, p. 253.

```{r}
solder <- expand_grid(cooling = c("H2O quench", "Oil quench"),
                      Sb = (c(3, 5, 10))) |> 
  mutate(k = 3) |> 
  uncount(k) |> 
  mutate(cooling = factor(cooling),
         Sb = factor(Sb, labels = c("3%", "5%", "10%")),
         strength = c(18.6, 19.5, 19.0, 22.3, 19.5, 20.5,
                      15.2, 17.1, 16.6, 20.0, 20.9, 20.4,
                      20.9, 22.9, 20.6, 16.4, 19.0, 18.1))
```

All responses

```{r}
mosaic::mean_(~ strength, data = solder)
```

### One factor, two levels

```{r}
solder |> 
  count(cooling)
# options("contrasts")
```

Means with two levels

Responses by cooling method

```{r}
mosaic::mean_(strength ~ cooling, data = solder)
```

With default coding

```{r}
lm_cool0 <- lm(strength ~ cooling, data = solder)
coef(lm_cool0)
```

With effects coding

```{r}
lm_cool1 <- lm(strength ~ cooling, data = solder,
               contrasts = list(cooling = "contr.sum"))
coef(lm_cool1)
```

Means and effects, default coding

```{r}
cooling_means0 <- emmeans::emmeans(lm_cool0, ~ cooling)
cooling_means0
emmeans::contrast(cooling_means0)
```

Means and effects, effect coding

```{r}
cooling_means1 <- emmeans::emmeans(lm_cool1, ~ cooling)
cooling_means1
emmeans::contrast(cooling_means1)
```

### One factor, more than two levels

```{r}
solder |>
  count(Sb)

mean(strength ~ Sb, data = solder)

# solder |> 
#   summarize(.by = Sb,
#             strength_by_Sb = mean(strength))
```

Responses by antimony content

```{r}
mosaic::mean_(strength ~ Sb, data = solder)
```

With default coding

```{r}
lm_Sb0 <- lm(strength ~ Sb, data = solder)
msummary(lm_Sb0)
emmeans::emmeans(lm_Sb0, ~ Sb)
emmeans::emmeans(lm_Sb0, ~ Sb) |> 
  emmeans::contrast()
```

With effects coding; suffix indicates level

```{r}
lm_Sb1 <- lm(strength ~ Sb, data = solder,
             contrasts = list(Sb = "contr.sum"))
msummary(lm_Sb1)
emmeans::emmeans(lm_Sb1, ~ Sb)
emmeans::emmeans(lm_Sb1, ~ Sb) |> 
  emmeans::contrast()
```

### Two factors, additive model

```{r}
lm_add <- lm(strength ~ cooling + Sb, data = solder,
             contrasts = list(cooling = "contr.sum",
                              Sb = "contr.sum"))
msummary(lm_add)
```

Interaction plot, additive model

```{r}
emmeans::emmip(lm_add, cooling ~ Sb)
```

### Two factors, full model with interaction

```{r}
lm_int <- lm(strength ~ cooling * Sb, data = solder,
             contrasts = list(cooling = "contr.sum",
                              Sb = "contr.sum"))
msummary(lm_int)
anova(lm_int)
```

### Means and effects, full model

```{r}
# table 5.4, p. 271
emmeans::emmeans(lm_int, ~ cooling + Sb) 

emmeans::emmeans(lm_int, ~ Sb + cooling) |> 
  contrast(interaction = TRUE)
```

```{r}
emmeans::emmip(lm_int, cooling ~ Sb)

```

Interaction plot, full model

```{r}
emmeans::emmip(lm_int, cooling ~ Sb)
```

### Example from student study

Change `wood` and `joint` variables to factors.

```{r}
wood_joints <- table4_11 |> 
  mutate(wood = factor(wood, levels = c("pine", "oak", "walnut")),
         joint = factor(joint, levels = c("butt", "beveled", "lap")))
dplyr::glimpse(wood_joints)
```

Find mean strength of all specimens.

```{r}
wood_joints |> 
  summarize(mean_strength = mean(strength))
```

Find sample means for all nine wood-joint combinations.

```{r}
options(digits = 4)
wood_joints |> 
  summarize(.by = c(joint, wood),
            mean_strength = mean(strength)
            )
```

Find mean of the nine combination means.

```{r}
options(digits = 5)
wood_joints |> 
  summarize(.by = c(joint, wood),
            mean_strength = mean(strength)) |> 
  summarize(cell_mean_strength = mean(mean_strength)
            )
```

Note difference due to unbalanced data.

Fit model with main effects only.

```{r}
wj_mod_main <- lm(strength ~ joint + wood, data = wood_joints,
                  contrasts = list(joint = contr.sum,
                                   wood = contr.sum))
msummary(wj_mod_main)

emmeans::emmeans(wj_mod_main, spec = ~ joint + wood) 
```

Interaction plot, main effects only.

```{r}
emmeans::emmip(wj_mod_main, joint ~ wood, col = "black",
      linearg = list(), dotarg = list(size = 2)) +
  scale_y_continuous(breaks = seq(600, 2400, 200)) +
  labs(x = "Wood", y = "Stress at failure (psi)")
```

Fit the full model with interaction.

```{r}
wj_mod <- lm(strength ~ joint * wood, data = wood_joints,
             contrasts = list(joint = contr.sum,
                              wood = contr.sum))
msummary(wj_mod)
emmeans::emmeans(wj_mod, spec = ~ joint + wood) 
```

Interaction plot, full model with interaction.

```{r}
emmeans::emmip(wj_mod, joint ~ wood, col = "black",
      linearg = list(), dotarg = list(size = 2)) +
  scale_y_continuous(breaks = seq(600, 2400, 200)) +
  labs(x = "Wood", y = "Stress at failure (psi)")
```

### 4.3.2: Simpler Descriptions for Some Two-Way Data Sets

Golf ball flight example

Make compression and evening factors

```{r}
golf <- table4_15 |> 
  mutate(across(c(compression, evening), factor))
```

Means for golf ball flight data

```{r}
mean(~ distance, data = golf)

mean(distance ~ compression, data = golf)

mean(distance ~ evening, data = golf)

mean(distance ~ compression + evening, data = golf)
```

Interaction plot, Fig. 4.25, p. 175

```{r}
lm_golf <- lm(distance ~ compression + evening, data = golf)
emmeans::emmip(lm_golf, compression ~ evening, 
               CIs =  TRUE,
               col = "black",
               linearg = list(), 
               dotarg = list(size = 2))

```

### 4.3.3: Fitted Effects for Three-Way (and Higher) Factorials

Kinzer data

```{r}
strength <- table4_20 |> 
  mutate(temp = factor(temp, labels = c("300F", "330F")),
         time = factor(time, labels = c("4hr", "12hr")),
         span = factor(span, labels = c("4hr", "12hr"))
         )

glimpse(strength)

material <- lm(strength_mean ~ temp * time * span, data = strength,
               contrasts = list(temp = "contr.sum",
                                time = "contr.sum",
                                span = "contr.sum")
               ) 

summary(material)
  
```

Cube plot, Figure 4.28, p. 181

```{r}
FrF2::cubePlot(material, "time", "span", "temp",  
               main = "Strength of Material", size = 0.22)

library(ggplot2)

# Sample 2^3 factorial data
# df <- data.frame(
#   A = c(-1, 1, -1, 1, -1, 1, -1, 1),
#   B = c(-1, -1, 1, 1, -1, -1, 1, 1),
#   C = c(-1, -1, -1, -1, 1, 1, 1, 1),
#   Response = c(10, 15, 12, 20, 18, 25, 22, 30)
# )
# 
# # Define 2D coordinates for the 3D cube visualization
# # This requires mapping the 8 corners (x, y)
# cube_coords <- data.frame(
#   df,
#   x = c(1, 2, 1, 2, 1.5, 2.5, 1.5, 2.5),
#   y = c(1, 1, 2, 2, 1.5, 1.5, 2.5, 2.5)
# )
# 
# # Plotting
# ggplot(cube_coords, aes(x=x, y=y)) +
#   # Draw lines connecting vertices (manual path ordering required)
#   geom_path(data=cube_coords[c(1,2,4,3,1,5,6,8,7,5,6,8,4,2,6,5,7,8,4,3,7),]) +
#   geom_point(size=5) +
#   geom_text(aes(label=Response), vjust=-1) +
#   theme_void() +
#   labs(title="Factorial Cube Plot")
```


