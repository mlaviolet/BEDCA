...
A straight line seems a reasonable fit. The `gf_lm()` operator adds the best fit line to the plot. We'll proceed to fit the model using the `lm()` function ("lm" stands for "linear model"). The syntax is
...
We can get more information using the `msummary()` operator from the `mosaic` package. The base R operator `summary()` produces a more extensive summary that we don't need yet.
...
```{r}
lm_density_aug <- broom::augment(lm_density)
lm_density_aug
```
...
R-squared gets its name because it's the square of the *correlation coefficient* $r$ between the observed values $y$ and the predictor values $x$.

The correlation coefficient measures the linear association between two quantitative variables and is computed with the R function `cor()`. For the pressure-density data
...
For models with more than one predictor, R-squared is the correlation between $y$ and the fitted values $\hat{y}$. We'll see examples shortly.
...
Plotting residuals is an important tool for checking whether a model is appropriate for the data. This is done in several ways.
...
-   *Normal plots of residuals*. Points should fall along a straight line, indicating that the points roughly follow the "normal" bell-shaped curve.
...
The function `gf_smooth()` below provides a general smooth.
...
The two methods produce different coefficients but the same predicted values. (For those who know linear algebra: the reason is that both sets of model vectors span the same vector space.)
...
As with any model, residual plots would be run to assess the adequacy of the model fit. Also, note that with 18 observations and four model terms, the total degrees of freedom are 17 (apart from the intercept), with three degrees of freedom for the model and 14 for residuals.
...
We'll call this model the *additive model*. For this model $R^2 = 0.394$ Next we'll construct an interaction plot using `tail` as the trace factor. Although `tail` is a quantitative variable, we need to make it a factor for the plot.
...
ggformula::gf_point(.fitted ~ canard, data = lm_add_aug,
  ...
  # enhancements
  scale_x_continuous(breaks = c(-1.2, 0, 1.2)) + # add canard values as ticks
...
As the authors note, residual plotting indicates that the model could be further refined.
...
Most of Section 4.2 discusses a data set about the operation of a plant for the oxidation of ammonia to nitric acid. The version used by the authors appears in Table 4.8 (p. 150) and in this package as `table4_08`. Variable included are

-   `air`: Flow of cooling air

-   `acid`: Concentration of acid (per 1000, minus 500)

-   `water`: Cooling water inlet temperature

-   `stack`: Stack loss, or ten times the percentage of ingoing ammonia that escapes from the absorption column unabsorbed (i.e., an inverse measure of overall plant efficiency).

The goal is to find a model describing how stack loss depends on air flow, acid concentration, and water inlet temperature. Since the investigator did not fully control the predictor variables, they are likely correlated. We can examine the correlations with the `cor()` operator.
...
**Note**: For exposition, we've included the intercept term in the model formula when using `lm()`. For instance, we fitted a line to the pressure-density data with

$$\texttt{lm(density}\sim\texttt{1 + pressure)}$$

We can also fit this model with the syntax

$$\texttt{lm(density}\sim\texttt{pressure)}$$

The intercept term is included unless explicitly omitted. To do that, replace 1 in the model with -1:

$$\texttt{lm(density}\sim\texttt{-1 + pressure)}$$

This model would force the regression line through the origin. Generally, you'll have no reason to do that unless a zero intercept makes practical sense *and* you have data where $x = 0$.

In fitting a model including two predictors and an interaction term, we used the syntax

$$\texttt{lm(lift_drag}\sim\texttt{1 + canard + tail + canard:tail}$$

We can use a shortened syntax

$$\texttt{lm(lift_drag}\sim\texttt{canard * tail)}$$

This code tells R to fit a model with an intercept, a term for each predictor, and an interaction term combining the two predictors.
...
The Greek letters are replaced by Latin letters and the response variable has a "hat" to indicate that $y$ is estimated based on the computed coefficients $b_0$ and $b_1$.
...
$$\texttt{Sum Sq(Residuals)} = \sum_{i=1}^{n}(y_i-\hat{y})^2$$ 
...
<https://cran.r-project.org/web/packages/sjPlot/vignettes/plot_interactions.html> The intercept term is included by default.
