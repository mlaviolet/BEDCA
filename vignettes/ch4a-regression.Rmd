...
The `Df` column sums to 14, although the sample size is 15. This is because the null model is considered the baseline model. The model degrees of freedom will always be the number of model terms less one.
...
We can easily obtain fitted values and residuals with the `augment()` function from the `broom` package. The following produces the fitted values and residuals from the pressure-density data (Table 4.5, p. 136).
...
-   Plots of residuals against variables (like machine number or operator) not used in the fitted equation but potentially of importance (use `data` = original_data)
