...
The `Laid` gears produce an average of `NA`, which means at least one value is missing. Let's make a count of "missing" measurements by treatment.

```{r}
mosaic::tally(is.na(Runout) ~ Treatment, data = gears)
```

We see that one laid gear is recorded as missing, so we have measurements for 39 hung gears and 38 laid gears. We'll resolve the issue by removing the missing value (since we don't know if it's really missing anyway).
...
We see that the runout for the hung gears exceeds that for the laid gears by 5.3 $\times$ 0.0001 inches. Let's make a comparative dot plot using `dotPlot()` from the `mosaic` package. The first argument is a formula where the variable behind the tilde (`~`) represents the varable to be plotted and the variable behind the vertical slash (`|`) specifies that the two graphs appear in separate panels. Formulas are discussed further in Chapter 3.
